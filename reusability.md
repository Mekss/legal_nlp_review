...
| **Bucket-3 extraction field** | a field name + one-paragraph definition + seed terms (`field_factory.ipynb`) | **review ~120 LLM-drafted labels (~1–2 h)** — the factory automates lexicon, detection, sampling and drafting; `field_deploy.ipynb` ships the calibrated column and `ask.ipynb` discovers it automatically. Flip rate reported as the anti-rubber-stamp check | the alienation field end-to-end (rules F1 0.507, LLM F1 0.593, ECE 0.179→0.068 OOF) + the factory demo field `supervised_contact` |
